...
# codehub-url-auth: credential helper registration is now optional.
# URL-embedded auth is used for clone; credential helper is only needed
# for subsequent push/pull operations. Set MYCO_USE_CREDENTIAL_HELPER=true
# to enable the helper (default: false, URL auth mode).
GIT_CRED_HELPER_PATH="/app/scripts/git-credential-myco.sh"
if [ "${MYCO_USE_CREDENTIAL_HELPER:-false}" = "true" ]; then
    if [ -x "$GIT_CRED_HELPER_PATH" ]; then
        git config --global credential.helper "$GIT_CRED_HELPER_PATH" 2>/dev/null || true
        echo "[entrypoint] registered git credential helper (optional mode) -> $GIT_CRED_HELPER_PATH" >&2
    else
        echo "[entrypoint] WARNING — credential helper missing or not executable at $GIT_CRED_HELPER_PATH" >&2
    fi
else
    echo "[entrypoint] skipping credential helper registration (URL auth mode)" >&2
fi
